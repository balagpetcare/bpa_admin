...
import { Card, Badge, Button, Form, Modal, Tabs, Tab, Alert, Spinner } from 'react-bootstrap'
import { Icon } from '@iconify/react'
import PageHeader from '@/components/ui/PageHeader'
import { contactInquiryApi, type InquiryDetail, type InquiryStatus, type InquiryPriority } from '@/lib/api/contact-inquiry.api'
...
export default function InquiryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    load()
    api
      .get<MailAccount[]>('/admin/mail/accounts')
  ...
  return (
    <div className="container-fluid">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Contact Inquiries', href: '/contact-inquiries' }, { label: inquiry.ticketNumber }]}
        action={
      ...
      <div className="row g-3">
            ...
            <Card.Body>
              ...
              {inquiry.organizationName && (
                <p className="text-muted small mb-1">
                  {inquiry.organizationName}
                  {inquiry.designation ? ` — ${inquiry.designation}` : ''}
                  {inquiry.website && (
                    <>
                      {' '}
                      ·{' '}
                      <a href={inquiry.website} target="_blank" rel="noreferrer">
                        {inquiry.website}
                      </a>
                    </>
                  )}
          ...
          <Tabs defaultActiveKey="replies" className="mb-3">
                ...
                inquiry.replies.map((r) => (
                  <Card key={r.id} className="mb-2">
                    <Card.Header className="small text-muted d-flex justify-content-between">
                      <span>
                        Sent by <strong>{r.sentBy.name}</strong> to {r.toAddresses.join(', ')}
                      </span>
              ...
              {inquiry.forwards.length === 0 ? (
                ...
                inquiry.forwards.map((f) => (
                  <Card key={f.id} className="mb-2">
                    <Card.Header className="small text-muted d-flex justify-content-between">
                      <span>
                        Forwarded by <strong>{f.forwardedBy.name}</strong> to {f.toAddresses.join(', ')}
                      </span>
                      <span>{new Date(f.forwardedAt).toLocaleString()}</span>
                    </Card.Header>
                    {f.note && (
                      <Card.Body>
                        <em className="text-muted small">Note: {f.note}</em>
                      </Card.Body>
                    )}
                  </Card>
                ))
            ...
            <Tab eventKey="notes" title={`Notes (${inquiry.internalNotes.length})`}>
              ...
              <div className="d-flex gap-2">
                <Form.Control as="textarea" rows={2} placeholder="Add internal note…" value={newNote} onChange={(e) => setNewNote(e.target.value)} />
        ...
        <div className="col-lg-4">
          <Card className="mb-3">
            <Card.Header>
              <strong>Contact Type &amp; Category</strong>
            </Card.Header>
            <Card.Body className="small">
              <div className="mb-1">
                <span className="text-muted">Type:</span> {inquiry.contactType?.labelEn ?? '—'}
              </div>
              <div>
                <span className="text-muted">Category:</span> {inquiry.category?.labelEn ?? '—'}
              </div>
            </Card.Body>
          </Card>
          ...
          <Card className="mb-3">
            ...
            <Card.Body className="small">
              <div className="mb-1">
                <span className="text-muted">Department:</span> {inquiry.department?.nameEn ?? '—'}
              </div>
              <div>
                <span className="text-muted">Assigned to:</span> {inquiry.assignedTo?.name ?? '—'}
              </div>
            </Card.Body>
          </Card>
          ...
          <Card>
            ...
            <Card.Body className="small">
              {inquiry.source && (
                <div className="mb-1">
                  <span className="text-muted">Source:</span> {inquiry.source}
                </div>
              )}
              {inquiry.ipAddress && (
                <div className="mb-1">
                  <span className="text-muted">IP:</span> {inquiry.ipAddress}
                </div>
              )}
              {inquiry.readAt && (
                <div className="mb-1">
                  <span className="text-muted">Read:</span> {new Date(inquiry.readAt).toLocaleString()}
                </div>
              )}
              {inquiry.resolvedAt && (
                <div className="mb-1">
                  <span className="text-muted">Resolved:</span> {new Date(inquiry.resolvedAt).toLocaleString()}
                </div>
              )}
              <div>
                <span className="text-muted">Consent:</span> {inquiry.consentGiven ? 'Yes' : 'No'}
              </div>
            </Card.Body>
          </Card>
        </div>
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              as="textarea"
              rows={8}
              value={replyData.bodyHtml}
              onChange={(e) => setReplyData((d) => ({ ...d, bodyHtml: e.target.value }))}
            />
          </Form.Group>
          <Form.Check
      ...
      <Modal show={fwdOpen} onHide={() => setFwdOpen(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>Forward Inquiry</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form.Group className="mb-3">
            <Form.Label>From Account</Form.Label>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              value={fwdData.to}
              onChange={(e) => setFwdData((d) => ({ ...d, to: e.target.value }))}
          ...
          <Form.Group>
            ...
            <Form.Control
              value={fwdData.note}
              onChange={(e) => setFwdData((d) => ({ ...d, note: e.target.value }))}
              placeholder="Why are you forwarding this?"
            />
          </Form.Group>
        </Modal.Body>
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setFwdOpen(false)}>
            Cancel
          </Button>
