...
const EMPTY: Partial<DonationImpactStory> = {
  titleEn: '', titleBn: '', slug: '', storyType: 'rescue',
  animalType: '', animalName: '', location: '',
  shortDescriptionEn: '', shortDescriptionBn: '',
  fullStoryEn: '', fullStoryBn: '',
  beforeImageUrl: '', afterImageUrl: '', costUsed: '',
  storyDate: '', isPublished: false, isFeatured: false,
}
...
export default function ImpactStoryForm({ storyId, initial }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    const result = isEdit
      ? await mutate(() => updateImpactStory(storyId, form), undefined)
      : await mutate(() => createImpactStory(form), undefined)
    ...
    <div className="container-fluid">
      ...
      <Form onSubmit={handleSubmit}>
        <Row className="g-4">
          <Col lg={8}>
            <Card className="mb-4">
              <Card.Header><h6 className="mb-0">Story Content</h6></Card.Header>
                ...
                <Row className="g-3">
                  <Col md={6}>
                    <Form.Group>
                      ...
                      <Form.Control
                        value={form.titleEn ?? ''}
                        onChange={(e) => { set('titleEn', e.target.value); if (!isEdit) set('slug', slugify(e.target.value)) }}
                        required
                      />
                    </Form.Group>
                  </Col>
                ...
                <Row className="g-3">
                  <Col md={6}>
                    <Form.Group>
                      ...
                      <Form.Control as="textarea" rows={2} value={form.shortDescriptionEn ?? ''} onChange={(e) => set('shortDescriptionEn', e.target.value)} />
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label>Short Description (BN)</Form.Label>
                      <Form.Control as="textarea" rows={2} value={form.shortDescriptionBn ?? ''} onChange={(e) => set('shortDescriptionBn', e.target.value)} />
                    </Form.Group>
                  </Col>
                </Row>
                <Form.Group>
                  <Form.Label>Full Story (English) <span className="text-danger">*</span></Form.Label>
            ...
            <Card>
              <Card.Header><h6 className="mb-0">Images</h6></Card.Header>
              <Card.Body>
                <Row className="g-3">
                  <Col md={6}>
                    <Form.Group>
                      <Form.Label>Before Image URL</Form.Label>
                      <Form.Control type="url" value={form.beforeImageUrl ?? ''} onChange={(e) => set('beforeImageUrl', e.target.value)} placeholder="https://..." />
                      {form.beforeImageUrl && (
                  ...
                  <Col md={6}>
                    <Form.Group>
                      ...
                      <Form.Control type="url" value={form.afterImageUrl ?? ''} onChange={(e) => set('afterImageUrl', e.target.value)} placeholder="https://..." />
                      {form.afterImageUrl && (
                        // eslint-disable-next-line @next/next/no-img-element
          ...
          <Col lg={4}>
            <Card className="mb-4">
              ...
              <Card.Body className="d-flex flex-column gap-3">
                <Form.Check type="switch" label="Published (visible on website)" checked={form.isPublished ?? false} onChange={(e) => set('isPublished', e.target.checked)} />
                ...
                <div className="d-flex gap-2">
                  <Button type="submit" variant="primary" disabled={loading} className="flex-1">
                    {loading ? <><span className="spinner-border spinner-border-sm me-1" />Saving…</> : isEdit ? 'Update Story' : 'Create Story'}
                  </Button>
                  <Button type="button" variant="outline-secondary" onClick={() => router.push('/donations/impact-stories')}>Cancel</Button>
                </div>
              </Card.Body>
            </Card>

            <Card className="mb-4">
              <Card.Header><h6 className="mb-0">Details</h6></Card.Header>
              <Card.Body className="d-flex flex-column gap-3">
                <Form.Group>
                  <Form.Label>Story Type <span className="text-danger">*</span></Form.Label>
                  <Form.Select value={form.storyType ?? 'rescue'} onChange={(e) => set('storyType', e.target.value)} required>
                    {STORY_TYPES.map((t) => <option key={t} value={t} className="text-capitalize">{t}</option>)}
                  </Form.Select>
                </Form.Group>
                <Row className="g-2">
                  <Col>
                ...
                <Form.Group>
                  ...
                  <Form.Control type="date" value={form.storyDate ? form.storyDate.slice(0, 10) : ''} onChange={(e) => set('storyDate', e.target.value)} />
                </Form.Group>
                <Form.Group>
            ...
            <Card>
              <Card.Header><h6 className="mb-0">Link to Campaign / Purpose</h6></Card.Header>
              <Card.Body className="d-flex flex-column gap-3">
                <Form.Group>
