...
export default function PetsContent() {
  ...
  function openCreateOwner() {
    ...
    setOwnerForm({ name: '', email: '', phone: '', address: '' })
  ...
  async function handleSaveOwner() {
    ...
    if (editingOwner) {
      await mutate(() => petsApi.updateOwner(editingOwner.id, dto), undefined)
    } else {
      await mutate(() => petsApi.createOwner(dto), undefined)
    }
    setShowModal(false)
    rOwners()
  }
  ...
  async function handleSavePet() {
    ...
    const dto = {
      ...
      microchipNumber: petForm.microchipNumber || undefined,
      notes: petForm.notes || undefined,
    }
    if (editingPet) {
      await mutate(() => petsApi.update(editingPet.id, dto), undefined)
    } else {
      await mutate(() => petsApi.create(dto), undefined)
    }
    setShowModal(false)
    rPets()
  }
  ...
  async function handleDelete(id: string, type: Tab) {
    ...
    if (type === 'owners') {
      await mutate(() => petsApi.deleteOwner(id), undefined)
      rOwners()
    } else {
      await mutate(() => petsApi.remove(id), undefined)
      rPets()
    }
  }
  ...
  return (
    <div className="container-fluid">
      ...
      <Nav variant="tabs" className="mb-3">
        <Nav.Item>
          <Nav.Link active={tab === 'owners'} onClick={() => switchTab('owners')}>
            Pet Owners
          </Nav.Link>
        </Nav.Item>
        <Nav.Item>
          <Nav.Link active={tab === 'pets'} onClick={() => switchTab('pets')}>
            Pets
          </Nav.Link>
        </Nav.Item>
      </Nav>
      ...
      <Card>
        <Card.Body>
                      ...
                      <tr key={o.id}>
                        ...
                        <td>
                          <Badge bg="primary-subtle" text="primary">
                            {o._count?.pets ?? 0} pets
                          </Badge>
                        </td>
                        <td className="text-end">
                          {can('pet_owners:update') && (
                            <Button variant="soft-primary" size="sm" className="me-1" onClick={() => openEditOwner(o)}>
                              <Icon icon="solar:pen-bold" />
                            </Button>
                          )}
                          {can('pet_owners:delete') && (
                            <Button variant="soft-danger" size="sm" onClick={() => handleDelete(o.id, 'owners')}>
                    ...
                    pets.map((p: Pet) => (
                      <tr key={p.id}>
                        <td>
                          ...
                          <div className="text-muted small">{p.owner?.name ?? '—'}</div>
                        </td>
                        ...
                        <td>
                          <span className={`badge bg-${p.isActive ? 'success' : 'secondary'}-subtle text-${p.isActive ? 'success' : 'secondary'}`}>
                            {p.isActive ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="text-end">
                          {can('pets:update') && (
                            <Button variant="soft-primary" size="sm" className="me-1" onClick={() => openEditPet(p)}>
                              <Icon icon="solar:pen-bold" />
                            </Button>
                          )}
                          {can('pets:delete') && (
                            <Button variant="soft-danger" size="sm" onClick={() => handleDelete(p.id, 'pets')}>
                              <Icon icon="solar:trash-bin-trash-bold" />
                            </Button>
                          )}
                        </td>
          ...
          {tab === 'owners' && ownerMeta && ownerMeta.totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <small className="text-muted">
                {ownerMeta.total} owners · Page {ownerMeta.page} of {ownerMeta.totalPages}
              </small>
              <div className="d-flex gap-1">
                <Button size="sm" variant="outline-secondary" disabled={!ownerMeta.hasPrev} onClick={() => setPage((p) => p - 1)}>
                  ‹
                </Button>
                <Button size="sm" variant="outline-secondary" disabled={!ownerMeta.hasNext} onClick={() => setPage((p) => p + 1)}>
                  ›
                </Button>
              </div>
            </div>
          ...
          {tab === 'pets' && petMeta && petMeta.totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <small className="text-muted">
                {petMeta.total} pets · Page {petMeta.page} of {petMeta.totalPages}
              </small>
              <div className="d-flex gap-1">
                <Button size="sm" variant="outline-secondary" disabled={!petMeta.hasPrev} onClick={() => setPage((p) => p - 1)}>
                  ‹
                </Button>
                <Button size="sm" variant="outline-secondary" disabled={!petMeta.hasNext} onClick={() => setPage((p) => p + 1)}>
                  ›
                </Button>
              </div>
            </div>
      ...
      {tab === 'owners' && (
        <Modal show={showModal} onHide={() => setShowModal(false)}>
          <Modal.Header closeButton>
            <Modal.Title>{editingOwner ? 'Edit' : 'Add'} Pet Owner</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form>
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={handleSaveOwner} disabled={saving}>
      ...
      {tab === 'pets' && (
        <Modal show={showModal} onHide={() => setShowModal(false)}>
          <Modal.Header closeButton>
            <Modal.Title>{editingPet ? 'Edit' : 'Add'} Pet</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <Form>
              ...
              <Row className="g-2 mb-3">
                ...
                <Col>
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => setPetForm((f) => ({ ...f, weightKg: e.target.value }))}
                  />
                </Col>
              </Row>
              <Form.Group className="mb-3">
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={handleSavePet} disabled={saving}>
              {saving ? 'Saving…' : editingPet ? 'Update' : 'Create'}
            </Button>
          </Modal.Footer>
        </Modal>
      )}
