  ...
  return (
    <div className="mb-3">
      <div className="d-flex justify-content-between align-items-center mb-1">
        ...
        <small className={`text-${variant} fw-semibold`}>
          {value} / {max} ({pct}%)
        </small>
      </div>
...
export default function CampaignAnalyticsDashboard({ campaignId }: { campaignId: string }) {
  ...
  return (
    <div className="container-fluid">
      <PageHeader
        title="Analytics"
        breadcrumbs={[{ label: 'Campaigns', href: '/campaigns' }, { label: 'Detail', href: `/campaigns/${campaignId}` }, { label: 'Analytics' }]}
      />
      ...
      <LoadingOverlay loading={sumLoading}>
        {c && (
          <>
            ...
            <Card className="mb-3">
              <Card.Header>
                <h6 className="mb-0">Campaign Health</h6>
              </Card.Header>
              <Card.Body>
                <Row className="g-3">
                  <Col md={6}>
            ...
            <Row className="g-3 mb-4">
              ...
              <Col xs={12} sm={6} xl={3}>
                <KpiCard
                  ...
                  label="Revenue (BDT)"
                  value={`৳${Number(c.totalRevenueBdt).toLocaleString()}`}
                  variant="success"
                />
              </Col>
              <Col xs={12} sm={6} xl={3}>
      ...
      <Row className="g-3">
        ...
        <Col lg={7}>
          <Card>
            <Card.Header>
              <h6 className="mb-0">Sessions Breakdown</h6>
            </Card.Header>
                    ...
                    {((sessions as unknown[]) ?? []).length === 0 ? (
                      ...
                      (
                        ...
                        <tr key={s.id}>
                          ...
                          <td>{s.bookedCount}</td>
                          <td>
                            <Badge bg="info">{s.vaccinated}</Badge>
                          </td>
                        </tr>
                      ))
                    )}
        ...
        <Col lg={5}>
          <Card>
            <Card.Header>
              <h6 className="mb-0">Vaccinations by Vaccine</h6>
            </Card.Header>
            <Card.Body className="p-0">
              <Table hover className="table-sm mb-0">
                <thead className="table-light">
                  ...
                  {((vacKpis as { byVaccine?: Array<{ vaccine: string; count: number }> })?.byVaccine ?? []).length === 0 ? (
                    ...
                    ((vacKpis as { byVaccine?: Array<{ vaccine: string; count: number }> })?.byVaccine ?? []).map((v) => (
                      <tr key={v.vaccine}>
                        <td>{v.vaccine}</td>
                        <td>
                          <Badge bg="primary">{v.count}</Badge>
                        </td>
                      </tr>
                    ))
                  )}
                  ...
                  <tbody>
                    {(
                      ...
                      <tr key={i}>
                        ...
                        <td>
                          <Badge bg="info">{d.vaccinationsAdministered}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
