...
export default function CampaignMediaManager({ campaignId }: Props) {
      ...
      <Card className="h-100 border-0 shadow-sm overflow-hidden mb-0">
        <Card.Header className="bg-white py-2 px-3 border-bottom d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center gap-2">
            ...
            {item && <Badge bg="success-subtle" text="success" pill className="fs-10">Set</Badge>}
          </div>
          {canEdit && !item && (
            <MediaPickerInput
              value={null}
              ...
              customTrigger={
                <Button size="sm" variant="primary" className="btn-xs py-1 px-2">
                  <Icon icon="solar:add-circle-bold" className="me-1" />Add
                </Button>
              }
            />
          )}
        ...
        <Card.Body className="p-0 bg-light-subtle d-flex flex-column">
          {!item ? (
            ...
            <>
              <div className="position-relative bg-light preview-container" 
                style={{ 
                ...
                {canEdit && (
                  <div className="position-absolute bottom-0 start-0 w-100 p-2 d-flex justify-content-center gap-1 bg-dark bg-opacity-25 backdrop-blur-sm">
                    ...
                    />
                    <Button size="sm" variant="light" className="btn-icon-xs shadow-sm" title="Crop"
                      onClick={() => setCropTarget({ item, role })}>
  ...
  const galleryItems = byRole('gallery').slice().sort((a, b) => a.sortOrder - b.sortOrder)
      ...
      <Card className="border-0 shadow-sm overflow-hidden mb-4">
        <Card.Header className="bg-white py-3 px-3 border-bottom d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center gap-2">
            ...
            <Badge bg="secondary-subtle" text="secondary" pill className="ms-1">{galleryItems.length}</Badge>
          </div>
          {canEdit && (
            <MediaPickerInput
              value={null}
              ...
              customTrigger={
                <Button size="sm" variant="primary">
                  <Icon icon="solar:add-circle-bold" className="me-1" />Add Images
                </Button>
              }
            />
          )}
                      ...
                      {canEdit && (
                        <div className="position-absolute bottom-0 start-0 w-100 p-1 d-flex justify-content-center gap-1 bg-dark bg-opacity-25 opacity-0 group-hover-visible">
                          <Button size="sm" variant="light" className="btn-xs py-0 px-1"
                            ...
                            onClick={(e) => { e.stopPropagation(); moveGalleryItem(item, 'up') }}>
                            <Icon icon="solar:arrow-left-bold" />
                          </Button>
                          <Button size="sm" variant="light" className="btn-xs py-0 px-1"
                            disabled={idx === galleryItems.length - 1 || mutating}
                            onClick={(e) => { e.stopPropagation(); moveGalleryItem(item, 'down') }}>
      ...
      {editingAlt && (
        <Modal show={true} onHide={() => setEditingAlt(null)} centered size="sm">
          ...
          <Modal.Footer className="py-2 px-3">
            <Button size="sm" variant="light" onClick={() => setEditingAlt(null)}>Cancel</Button>
            <Button size="sm" variant="primary" onClick={() => handleUpdateAltText(items!.find(i => i.id === editingAlt)!)}>Save</Button>
          </Modal.Footer>
        </Modal>
      )}
