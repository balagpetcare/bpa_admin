...
export default function StaffAssignment({ campaignId }: { campaignId: string }) {
  ...
  function openAddModal() {
    setEditingId(null)
    setUserId(''); setDutyRole('GENERAL_VOLUNTEER'); setSessionId(''); setNotes('')
    setUserSearch(''); setSuccessMsg(null); clearError()
    setShowModal(true)
  }

  function openEditModal(a: CampaignStaffAssignment) {
    setEditingId(a.id)
    setUserId(a.userId); setDutyRole(a.dutyRole); setSessionId(a.sessionId ?? ''); setNotes(a.notes ?? '')
  ...
  async function handleSave() {
    ...
    clearError(); setSuccessMsg(null)

    let result
    if (editingId) {
      result = await mutate(() => campaignsApi.updateStaffAssignment(campaignId, editingId, {
        dutyRole, sessionId: sessionId || null, notes: notes || null
      }), undefined)
    } else {
      result = await mutate(() => campaignsApi.assignStaff(campaignId, {
        userId, dutyRole, sessionId: sessionId || undefined, notes: notes || null
      }), undefined)
    }
  ...
  async function handleDeactivate(id: string) {
    ...
    if (result !== null) { setSuccessMsg('Assignment deactivated.'); refetch() }
  }

  async function handleBulkAssign() {
    if (bulkUserIds.length === 0) return
    clearError(); setSuccessMsg(null)
    const result = await mutate(() => campaignsApi.bulkAssignStaff(campaignId, {
      assignments: bulkUserIds.map(uid => ({ userId: uid, dutyRole: bulkDuty, sessionId: bulkSessionId || undefined }))
    }), undefined)
    if (result) {
      setShowBulkModal(false)
      setBulkUserIds([]); setBulkSearch('')
      ...
      <PageHeader
        ...
        action={canAssign ? (
          <div className="d-flex gap-2">
            <Button variant="outline-primary" size="sm" onClick={() => { setBulkUserIds([]); setBulkSearch(''); setBulkDuty('GENERAL_VOLUNTEER'); setBulkSessionId(''); clearError(); setShowBulkModal(true) }}>
              <Icon icon="solar:users-group-two-rounded-bold" className="me-1" />Bulk Assign
            </Button>
            <Button variant="primary" onClick={openAddModal}>
              <Icon icon="solar:add-circle-bold" className="me-1" />Assign Staff
            </Button>
          </div>
        ) : undefined}
      />
            ...
            return count > 0 ? (
              <Col key={d.value} xs={6} sm={4} md={2}>
                <Card className="text-center py-2 px-1 border-0 shadow-sm">
                  <div className={`text-${d.color} fs-4`}><Icon icon={d.icon} /></div>
              ...
              <tbody>
                {assignments.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-5 text-muted">
                    ...
                    {canAssign && <Button variant="outline-primary" size="sm" className="mt-2" onClick={openAddModal}>Assign first staff member</Button>}
                  </td></tr>
                ) : assignments.map((a) => (
                  <tr key={a.id} className={!a.isActive ? 'opacity-50' : ''}>
                    <td>
                      ...
                      {a.user?.userRoles?.length > 0 && (
                        <div className="mt-1">{getUserRoles(a.user).split(', ').map(r => (
                          <span key={r} className="badge bg-light text-dark me-1 border">{r.replace('_', ' ')}</span>
                        ))}</div>
                      )}
                    </td>
                    <td><DutyBadge duty={a.dutyRole} /></td>
                    <td>
                      {a.session ? (
                        <div>
                          <div className="fw-semibold small">{dayjs(a.session.sessionDate).format('DD MMM YYYY')}</div>
                          <div className="text-muted small">{a.session.startTime} · {a.session.venue?.name}</div>
                        </div>
        ...
        <Modal.Body>
          ...
          {!editingId && (
            <Form.Group className="mb-3">
              <Form.Label className="fw-semibold">Search User <span className="text-danger">*</span></Form.Label>
              <Form.Control
                type="text"
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            {bulkUserIds.length > 0 && (
              <div className="mt-1 text-success small"><Icon icon="solar:check-circle-bold" className="me-1" />{bulkUserIds.length} user(s) selected</div>
            )}
          </Form.Group>
