...
function DutyBadge({ duty, isSigningDoctor, isPrimarySupervisor }: { duty?: DoctorDutyRole | null; isSigningDoctor?: boolean; isPrimarySupervisor?: boolean }) {
  ...
  return (
    <div className="d-flex flex-wrap gap-1">
      ...
      {isSigningDoctor && <span className="badge bg-danger text-white"><Icon icon="solar:pen-bold" className="me-1" />Signing</span>}
      {isPrimarySupervisor && <span className="badge bg-warning text-dark"><Icon icon="solar:crown-bold" className="me-1" />Primary Supervisor</span>}
    </div>
  )
}
...
export default function DoctorsAssignment({ campaignId }: { campaignId: string }) {
  ...
  const availableFn = useCallback(() => campaignsApi.getAvailableDoctors(campaignId, {
    search: debouncedSearch || undefined, limit: 100, includeAssigned: true
  ...
  const bulkAvailFn = useCallback(() => campaignsApi.getAvailableDoctors(campaignId, {
    search: debouncedBulkSearch || undefined, limit: 100, includeAssigned: true
  ...
  function hasSigningDoctor(forSessionId?: string) {
    return assignedList.some(a =>
      a.isSigningDoctor && (forSessionId ? a.sessionId === forSessionId : !a.sessionId)
    )
  }

  function openAddModal() {
    setEditingId(null)
    setDoctorId(''); setRole('vaccinator'); setDoctorDuty('VACCINATOR')
    setIsSigningDoctor(false); setIsPrimarySupervisor(false)
    setSessionId(''); setNotes(''); setSearch(''); setSuccessMsg(null); clearError()
    setShowModal(true)
  }

  function openEditModal(a: CampaignDoctor) {
    setEditingId(a.id)
    setDoctorId(a.doctorId); setRole(a.role ?? 'vaccinator')
    ...
    setSessionId(a.sessionId ?? ''); setNotes(a.notes ?? '')
  ...
  async function handleSave() {
    ...
    let result
    if (editingId) {
      result = await mutate(() => campaignsApi.updateDoctorAssignment(campaignId, editingId, {
        role, doctorDuty, isSigningDoctor, isPrimarySupervisor, notes: notes || null
      }), undefined)
    } else {
      result = await mutate(() => campaignsApi.assignDoctor(campaignId, {
        doctorId, role, doctorDuty, isSigningDoctor, isPrimarySupervisor,
        sessionId: sessionId || undefined, notes: notes || null
      }), undefined)
    }
  ...
  async function handleBulkAssign() {
    ...
    setSuccessMsg(null); clearError()
    const result = await mutate(() => campaignsApi.bulkAssignDoctors(campaignId, {
      assignments: bulkDoctorIds.map(did => ({
        doctorId: did, doctorDuty: bulkDuty,
        sessionId: bulkSessionId || undefined
      }))
    }), undefined)
    if (result) {
      setShowBulkModal(false)
      setBulkDoctorIds([]); setBulkSearch('')
      ...
      <PageHeader
        ...
        action={canAssign ? (
          <div className="d-flex gap-2">
            <Button variant="outline-primary" size="sm" onClick={() => { setBulkDoctorIds([]); setBulkSearch(''); setBulkDuty('VACCINATOR'); setBulkSessionId(''); clearError(); setShowBulkModal(true) }}>
              <Icon icon="solar:users-group-two-rounded-bold" className="me-1" />Bulk Assign
            </Button>
      ...
      {signingDoctors.length === 0 && (
        <Alert variant="warning" className="d-flex align-items-center gap-2">
          ...
          <div>
            ...
            {canAssign && <Button variant="link" size="sm" className="ms-1 p-0" onClick={openAddModal}>Assign signing doctor →</Button>}
          </div>
        </Alert>
      )}
              ...
              <tbody>
                {assignedList.length === 0 ? (
                  <tr><td colSpan={6} className="text-center py-5 text-muted">
                    ...
                    {canAssign && <Button variant="outline-primary" size="sm" className="mt-2" onClick={openAddModal}>Assign first doctor</Button>}
                  </td></tr>
                    ...
                    <td>
                      {a.session ? (
                        <div>
                          ...
                          <div className="text-muted" style={{ fontSize: '0.75rem' }}>{a.session.startTime} · {a.session.venue?.name}</div>
                        </div>
        ...
        <Modal.Body>
          ...
          {!editingId && (
            <Form.Group className="mb-3">
              <Form.Label className="fw-semibold">Search Doctor <span className="text-danger">*</span></Form.Label>
              <div className="d-flex gap-2 mb-2">
                <Form.Control type="text" placeholder="Search by name, phone, email, or license number..."
              ...
              <div className="border rounded bg-light p-2" style={{ maxHeight: 200, overflowY: 'auto' }}>
                {loadingAvailable ? (
                  ...
                  <div className="text-center text-muted py-3 small">
                    No doctors found.
                    <Button size="sm" variant="link" className="ms-1" onClick={() => window.open('/doctors', '_blank')}>Add a doctor</Button>
                  </div>
                  ...
                  return (
                    <button key={d.id} type="button"
                      ...
                      <div className="fw-semibold">
                        {d.name}
                        {d.specialization && <span className={`ms-2 badge ${isSelected ? 'bg-white text-primary' : 'bg-secondary-subtle text-secondary'}`}>{d.specialization}</span>}
                      </div>
                      <div className={`small ${isSelected ? 'text-white-50' : 'text-muted'}`}>
                        License: {d.licenseNumber || '—'}{d.phone ? ` · ${d.phone}` : ''}{d.email ? ` · ${d.email}` : ''}
                      </div>
                    </button>
                  )
                })}
          ...
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">Duty Role <span className="text-danger">*</span></Form.Label>
                <Form.Select value={doctorDuty} onChange={e => {
                  const val = e.target.value as DoctorDutyRole
                  setDoctorDuty(val)
                  setIsSigningDoctor(val === 'SIGNING_DOCTOR')
                }}>
                  {DUTY_ROLES.map(d => <option key={d.value} value={d.value}>{d.label}</option>)}
                </Form.Select>
              </Form.Group>
            </Col>
            <Col md={6}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Select value={sessionId} onChange={e => setSessionId(e.target.value)}>
                    ...
                    {(sessions ?? []).map((s: CampaignSession) => (
                      <option key={s.id} value={s.id}>{dayjs(s.sessionDate).format('DD MMM YYYY')} · {s.startTime} · {s.venue?.name}</option>
                    ))}
                  </Form.Select>
                </Form.Group>
              )}
          ...
          <Row>
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Check type="checkbox" id="isSigningDoctor" label="Signing Doctor (can sign certificates)"
                  checked={isSigningDoctor} onChange={e => setIsSigningDoctor(e.target.checked)}
                  className="mb-1"
                />
                <Form.Check type="checkbox" id="isPrimarySupervisor" label="Primary Supervisor"
                  checked={isPrimarySupervisor} onChange={e => setIsPrimarySupervisor(e.target.checked)}
                />
              </Form.Group>
            </Col>
          </Row>
      ...
      <Modal show={showBulkModal} onHide={() => setShowBulkModal(false)} size="lg">
        ...
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control type="text" placeholder="Search by name, license, email..."
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="text-muted" style={{ fontSize: '0.75rem' }}>License: {d.licenseNumber || '—'}{d.specialization ? ` · ${d.specialization}` : ''}</div>
                    </div>
                  </div>
                )
              })}
            ...
            {bulkDoctorIds.length > 0 && (
              <div className="mt-1 text-success small"><Icon icon="solar:check-circle-bold" className="me-1" />{bulkDoctorIds.length} doctor(s) selected</div>
            )}
          </Form.Group>
