  ...
  function openVaccinateModal(booking: PetBookingDetail) {
    setVaccinateForm({
      ...
      services: booking.services
        ...
        .map(s => ({ campaignServiceId: s.campaignServiceId, name: s.campaignService.name, isRequired: s.campaignService.isRequired, batchNumber: '' })),
      doctorId: '',
      notes: '',
    })
  }
          ...
          <Row className="g-2 align-items-end">
            ...
            <Col md="auto">
              <Button variant="primary" onClick={handleSearch} disabled={loading}>
                <Icon icon="solar:magnifer-bold" className="me-1" />Search
              </Button>
            </Col>
            <Col md="auto">
              <Button variant="outline-secondary" onClick={handleScanQr} disabled={loading}>
                <Icon icon="solar:qr-code-bold" className="me-1" />Scan QR
              </Button>
            </Col>
          </Row>
          {searchError && <Alert variant="warning" className="mt-2 mb-0">{searchError}</Alert>}
        </Card.Body>
      </Card>
      ...
      <Row className="g-3">
        ...
        {results.length > 0 && (
          <Col md={selected ? 5 : 12}>
            <Card>
              <Card.Header><h6 className="mb-0">Results ({results.length})</h6></Card.Header>
              <Card.Body className="p-0">
                <Table hover className="mb-0">
                  <thead className="table-light">
                    <tr><th>Pet</th><th>Owner</th><th>Booking #</th><th>Status</th><th /></tr>
                  </thead>
                  <tbody>
                    {results.map(b => (
                      <tr key={b.id} className={selected?.id === b.id ? 'table-active' : ''} style={{ cursor: 'pointer' }} onClick={() => setSelected(b)}>
                        <td className="fw-semibold">{b.pet.name} <small className="text-muted text-capitalize">({b.pet.petType})</small></td>
                        <td>{b.pet.owner?.ownerName}<br /><small className="text-muted">{b.pet.owner?.mobile}</small></td>
                        <td><code>{b.registration.bookingNumber}</code></td>
                        <td><Badge bg={STATUS_COLORS[b.status]} className="text-capitalize">{b.status.replace(/_/g, ' ')}</Badge></td>
                        <td><Button size="sm" variant="outline-primary" onClick={(e) => { e.stopPropagation(); setSelected(b) }}><Icon icon="solar:eye-bold" /></Button></td>
                      </tr>
                    ))}
                  </tbody>
                </Table>
          ...
          <Col md={results.length > 0 ? 7 : 12}>
            <LoadingOverlay loading={actionLoading}>
              <BookingDetailCard
                ...
                onClear={() => { setSelected(null); setResults([]) }}
              />
            </LoadingOverlay>
          </Col>
        )}
        ...
        <Modal show onHide={() => setVaccinateForm(null)} size="lg">
          ...
          <Modal.Body>
            <Table bordered size="sm" className="mb-3">
              <thead className="table-light">
                <tr><th>Service</th><th>Required</th><th>Batch Number</th></tr>
              </thead>
              <tbody>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={() => setVaccinateForm(null)}>Cancel</Button>
...
function BookingDetailCard({
  booking, canCheckIn, onCheckIn, onVaccinate, onClear,
}: {
  ...
  return (
    <Card>
      <Card.Header className="d-flex justify-content-between align-items-center">
        <h6 className="mb-0">{booking.pet.name} <span className="text-muted fw-normal text-capitalize">({booking.pet.petType})</span></h6>
        <Button variant="outline-secondary" size="sm" onClick={onClear}><Icon icon="solar:close-circle-bold" /></Button>
      </Card.Header>
      <Card.Body>
        <Row className="g-3 mb-3">
          <Col xs={6}>
            <small className="text-muted d-block">Status</small>
            <Badge bg={STATUS_COLORS[booking.status]} className="text-capitalize">{booking.status.replace(/_/g, ' ')}</Badge>
          </Col>
          <Col xs={6}>
          ...
          <Col xs={6}>
            ...
            <small className="text-muted">{booking.session.startTime} · {booking.session.venue?.name ?? '—'}</small>
          </Col>
          {booking.checkedInAt && (
            <Col xs={6}><small className="text-muted d-block">Checked In</small>{new Date(booking.checkedInAt).toLocaleTimeString()}</Col>
          )}
          {booking.vaccinatedAt && (
            <Col xs={6}><small className="text-muted d-block">Vaccinated</small>{new Date(booking.vaccinatedAt).toLocaleTimeString()}</Col>
          )}
        </Row>
        ...
        <div className="d-flex gap-2 flex-wrap">
          {canDoCheckIn && (
            <Button variant="success" onClick={onCheckIn}>
              <Icon icon="solar:check-circle-bold" className="me-1" />Check In
            </Button>
          )}
          {canDoVaccinate && (
            <Button variant="primary" onClick={onVaccinate}>
              <Icon icon="solar:syringe-bold" className="me-1" />Mark Vaccinated ({unadministered.length})
            </Button>
          )}
          {!canDoCheckIn && !canDoVaccinate && (
            <span className="text-muted fst-italic">
              {booking.status === 'vaccinated' ? 'All vaccinations completed.' :
               booking.status === 'cancelled' ? 'Booking is cancelled.' :
               booking.status === 'pending_payment' ? 'Awaiting payment.' :
               administered.length > 0 ? `${administered.length} service(s) administered.` : 'No actions available.'}
            </span>
          )}
        </div>
      </Card.Body>
