...
export default function CoverageAreasManager({ campaignId }: { campaignId: string }) {
  ...
  useEffect(() => {
    if (!debouncedSearch.trim() || isNationwide) {
      setSearchResults([])
      return
    }
    setSearching(true)
    locationTreeApi
      .search(debouncedSearch.trim(), level)
  ...
  return (
    <div className="container-fluid">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Campaigns', href: '/campaigns' }, { label: 'Detail', href: `/campaigns/${campaignId}` }, { label: 'Coverage Areas' }]}
        action={
          canManage ? (
      ...
      <Alert variant="info" className="d-flex align-items-start gap-2 py-2 small">
        ...
        <div>
          Public visitors pick their location first (Union/Upazila/District/Division or Ward/Zone/City Corporation), and see this campaign if it
          covers that exact area <strong>or the nearest broader area you assign here</strong>. A campaign with no coverage anywhere is invisible on
          the public site.
        </div>
      </Alert>
                ...
                {coverageList.length === 0 ? (
                  ...
                  coverageList.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>
                        <Badge
                          bg={c.isNationwide || true ? 'success-subtle' : 'secondary-subtle'}
                          text={c.isNationwide || true ? 'success' : 'secondary'}>
                          Active
                        </Badge>
                      </td>
                      {canManage && (
                        <td className="text-end">
                          {!c.isNationwide && c.location && VENUE_ELIGIBLE_TYPES.includes(c.location.type) && (
                            <Link
                              href={`/venues?locationId=${c.location.id}`}
                              className="btn btn-outline-secondary btn-sm me-1"
                              title="Manage venues here">
                              <Icon icon="solar:buildings-2-bold-duotone" className="me-1" />
                              Venues
                            </Link>
                          )}
            ...
            <>
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  value={level}
                  onChange={(e) => {
                    setLevel(e.target.value as LocationNodeType)
                    setSelectedNode(null)
                    setSearchResults([])
                  }}>
              ...
              <Form.Group className="mb-3">
                <Form.Label className="fw-semibold">
                  Search &amp; Select Area <span className="text-danger">*</span>
                </Form.Label>
                <Form.Control
                  type="text"
                ...
                <Form.Text className="text-muted">
                  This only selects an existing area from the location tree — new Divisions/Districts/Upazilas/Unions are created in the Locations
                  admin section, never here.
                </Form.Text>
              </Form.Group>
            </>
          )}
