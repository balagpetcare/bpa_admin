...
export default function CertificatesManager({ campaignId }: { campaignId: string }) {
  ...
  const fetchFn = useCallback(() => certificatesApi.list({ campaignId, page, limit: 20 }), [campaignId, page])
  ...
  return (
    <div className="container-fluid">
      <PageHeader
        title="Certificates"
        breadcrumbs={[{ label: 'Campaigns', href: '/campaigns' }, { label: 'Detail', href: `/campaigns/${campaignId}` }, { label: 'Certificates' }]}
        action={
                      ...
                      <td>
                        <div className="d-flex gap-1">
                          ...
                          {can('campaign_certificates:issue') && !cert.supersededAt && (
                            <Button size="sm" variant="soft-warning" onClick={() => handleReissue(cert.petBookingId)} disabled={mutating}>
          ...
          {searchResult &&
            (searchResult.valid && searchResult.certificate ? (
              <Alert variant="success">
                ...
                <div>
                  Pet: {searchResult.certificate.petBooking.pet.name} ({searchResult.certificate.petBooking.pet.petType})
                </div>
