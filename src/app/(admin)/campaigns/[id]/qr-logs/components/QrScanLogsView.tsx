...
export default function QrScanLogsView({ campaignId }: { campaignId: string }) {
  ...
  const fetchFn = useCallback(() => campaignAnalyticsApi.getQrScanLogs(campaignId, { page, limit: 30 }), [campaignId, page])
  ...
  return (
    <div className="container-fluid">
      <PageHeader
        title="QR Scan Logs"
        breadcrumbs={[{ label: 'Campaigns', href: '/campaigns' }, { label: 'Detail', href: `/campaigns/${campaignId}` }, { label: 'QR Logs' }]}
      />
                  ...
                  items.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td>
                        {log.petBooking ? (
                          <>
                            <span className="fw-semibold">{log.petBooking.pet.name}</span>{' '}
                            <small className="text-muted text-capitalize">({log.petBooking.pet.petType})</small>
                          </>
                        ) : (
                      ...
                      <td>
                        {log.petBooking ? (
                          <Badge bg="secondary" className="text-capitalize">
                            {log.petBooking.status.replace('_', ' ')}
                          </Badge>
                        ) : (
                          <span className="text-muted">—</span>
                        )}
                      ...
                      <td>
                        <Badge bg={log.scanResult === 'found' ? 'success' : 'danger'}>{log.scanResult}</Badge>
                      </td>
                      <td>
