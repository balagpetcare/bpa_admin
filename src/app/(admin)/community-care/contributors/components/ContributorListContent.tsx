...
export default function ContributorListContent() {
  ...
  const fetchFn = useCallback(() => careContributionsApi.list({ page, limit: 20, status: status || undefined }), [page, status])
  ...
  return (
    <div className="container-fluid">
      <PageHeader title="Contributors" breadcrumbs={[{ label: 'Community Care Fund' }, { label: 'Contributors' }]} />
                  ...
                  contributions.map((c: CareContribution) => (
                    <tr key={c.id} style={{ cursor: 'pointer' }} onClick={() => router.push(`/community-care/contributors/${c.id}`)}>
                      ...
                      <td className="text-end" onClick={(e) => e.stopPropagation()}>
                        <Button variant="soft-primary" size="sm" onClick={() => router.push(`/community-care/contributors/${c.id}`)}>
          ...
          {meta && meta.totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <small className="text-muted">
                {meta.total} contributions · Page {meta.page} of {meta.totalPages}
              </small>
