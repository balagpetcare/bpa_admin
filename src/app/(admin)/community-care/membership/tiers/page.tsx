        ...
        <tbody>
          {tiers.map((tier, index) => (
            <tr key={tier.id}>
              ...
              <td>{tier.nameEn}</td>
              <td>{tier.nameBn}</td>
              <td><Badge bg="secondary">{tier.slug}</Badge></td>
              ...
              <td>{tier.petLimitMin}-{tier.includedPets}-{tier.petLimitMax}</td>
              <td><Badge bg={tier.isActive ? 'success' : 'secondary'}>{tier.status ?? (tier.isActive ? 'active' : 'inactive')}</Badge></td>
              <td>
                <Button as={Link as any} href={`/community-care/membership/tiers/${tier.id}/edit`} size="sm" variant="info" className="me-1">Edit</Button>
                <Button size="sm" variant="danger" onClick={() => handleDelete(tier.id)}>Delete</Button>
              </td>
            </tr>
          ))}
        </tbody>
