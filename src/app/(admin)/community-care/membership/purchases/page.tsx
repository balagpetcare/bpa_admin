    ...
    <>
      ...
      <Card className="mb-3">
        <Card.Body>
          <Row className="g-2">
            <Col md={4}>
              <Form.Control placeholder="Search name/mobile/card..." value={search} onChange={(e) => setSearch(e.target.value)} />
            </Col>
            <Col md={3}>
            ...
            <tbody>
              {purchases.map((p: any, i: number) => (
                <tr key={p.id}>
                  <td>{i + 1}</td>
                  <td>{p.memberName}</td>
                  <td>{p.memberMobile}</td>
                  <td>
                    <Badge bg="info">{p.tier?.nameEn}</Badge>
                  </td>
                  ...
                  <td>
                    <Button as={Link as any} href={`/community-care/membership/purchases/${p.id}`} size="sm" variant="info">
                      View
                    </Button>
                  </td>
                </tr>
          ...
          {meta.totalPages > 1 && (
            <div className="d-flex justify-content-center gap-2 mt-3">
              {Array.from({ length: meta.totalPages }, (_, i) => (
                <Button key={i} size="sm" variant={meta.page === i + 1 ? 'primary' : 'outline-primary'} onClick={() => fetch(i + 1)}>
                  {i + 1}
                </Button>
              ))}
            </div>
