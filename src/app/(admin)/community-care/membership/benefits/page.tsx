...
export default function BenefitsPage() {
  ...
  const openEdit = (b: any) => {
    ...
    setForm({
      ...
      isActive: b.isActive,
      tierIds: b.tierMappings?.map((m: any) => m.tierId) || [],
    })
    setShowModal(true)
  }
    ...
    <>
      <PageHeader
        ...
        action={
          <Button
            ...
            onClick={() => {
              setEditBenefit(null)
              setForm({ titleEn: '', titleBn: '', descriptionEn: '', descriptionBn: '', icon: '', sortOrder: 0, isActive: true, tierIds: [] })
              setShowModal(true)
            }}>
            + Add Benefit
          </Button>
            ...
            <tbody>
              {benefits.map((b: any, i: number) => (
                <tr key={b.id}>
                  ...
                  <td>
                    <Badge bg={b.isActive ? 'success' : 'secondary'}>{b.isActive ? 'Active' : 'Inactive'}</Badge>
                  </td>
                  <td>
                    <Button size="sm" variant="info" className="me-1" onClick={() => openEdit(b)}>
                      Edit
                    </Button>
                    <Button
                      size="sm"
                      variant="danger"
                      onClick={async () => {
                        await communityMembershipApi.deleteBenefit(b.id)
                        fetch()
                      }}>
                      Delete
                    </Button>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} size="lg">
        <Modal.Header closeButton>
          <Modal.Title>{editBenefit ? 'Edit Benefit' : 'Add Benefit'}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Row className="g-3">
            <Col md={6}>
              <Form.Group>
                ...
                <Form.Control value={form.titleEn} onChange={(e) => setForm({ ...form, titleEn: e.target.value })} />
              </Form.Group>
            </Col>
            <Col md={6}>
              <Form.Group>
                <Form.Label>Title (BN)</Form.Label>
                <Form.Control value={form.titleBn} onChange={(e) => setForm({ ...form, titleBn: e.target.value })} />
              </Form.Group>
            </Col>
            <Col md={6}>
              <Form.Group>
            ...
            <Col md={4}>
              <Form.Group>
                ...
                <Form.Control value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} />
              </Form.Group>
            </Col>
            <Col md={4}>
              <Form.Group>
            ...
            <Col md={12}>
              <Form.Group>
                <Form.Label>Assign to Tiers</Form.Label>
                <div>
                  {tiers.map((t: any) => (
                    <Form.Check
                      key={t.id}
                      ...
                      onChange={(e) =>
                        setForm({ ...form, tierIds: e.target.checked ? [...form.tierIds, t.id] : form.tierIds.filter((id) => id !== t.id) })
                      }
                    />
                  ))}
                </div>
