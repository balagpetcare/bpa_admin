    ...
    <Modal show={show} onHide={onHide} size="lg">
      ...
      <Modal.Body>
        <Row className="g-3">
          ...
          <Col md={6}><Form.Label>Role</Form.Label><Form.Select value={form.role ?? 'gallery'} onChange={(e) => setForm((s: any) => ({ ...s, role: e.target.value }))}><option value="gallery">Gallery</option><option value="hero">Hero</option><option value="mobile_banner">Mobile Banner</option><option value="thumbnail">Thumbnail</option><option value="video_poster">Video</option></Form.Select></Col>
          <Col md={12}><MediaPickerInput value={form.mediaFileId} previewUrl={form.previewUrl} previewMimeType={form.previewMimeType} onChange={(fileId, file) => setForm((s: any) => ({ ...s, mediaFileId: fileId, previewUrl: file?.url ?? null, previewMimeType: file?.mimeType ?? null }))} label="Media File" mimeTypePrefix={form.role === 'video_poster' ? 'video/' : 'image/'} accept={form.role === 'video_poster' ? 'video/*' : 'image/*'} helpText="Upload a valid image or video file. Use Replace if the selected file is invalid." maxFileSizeBytes={5 * 1024 * 1024} /></Col>
          ...
          <Col md={4}><Form.Label>Alt Text</Form.Label><Form.Control value={form.altText ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, altText: e.target.value }))} /></Col>
          <Col md={6}><Form.Label>Sort Order</Form.Label><Form.Control value={form.sortOrder ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, sortOrder: e.target.value }))} /></Col>
      ...
      <Modal.Footer><Button variant="light" onClick={onHide}>Cancel</Button><Button onClick={save} disabled={!form.mediaFileId}>Save</Button></Modal.Footer>
    </Modal>
  )
}
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      ...
      <Modal.Body>
        <Row className="g-3">
          <Col md={6}><Form.Label>Campaign</Form.Label><Form.Select value={form.campaignId ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, campaignId: e.target.value }))}>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.titleEn}</option>)}</Form.Select></Col>
          <Col md={6}><Form.Label>Document Type</Form.Label><Form.Control value={form.documentType ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, documentType: e.target.value }))} /></Col>
          <Col md={12}><MediaPickerInput value={form.mediaFileId} previewUrl={form.previewUrl} previewMimeType={form.previewMimeType} onChange={(fileId, file) => setForm((s: any) => ({ ...s, mediaFileId: fileId, previewUrl: file?.url ?? null, previewMimeType: file?.mimeType ?? null }))} label="Document File" emptyLabel="Select document" mimeTypePrefix="application/" accept="application/*" maxFileSizeBytes={15 * 1024 * 1024} /></Col>
          ...
          <Col md={6}><Form.Label>External URL</Form.Label><Form.Control value={form.fileUrl ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, fileUrl: e.target.value }))} /></Col>
          <Col md={3}><Form.Label>Sort Order</Form.Label><Form.Control value={form.sortOrder ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, sortOrder: e.target.value }))} /></Col>
