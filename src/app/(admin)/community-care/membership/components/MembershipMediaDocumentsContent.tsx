  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{item ? 'Edit Media' : 'Add Media'}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Row className="g-3">
          <Col md={6}>
            <Form.Label>Campaign</Form.Label>
            <Form.Select value={form.campaignId ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, campaignId: e.target.value }))}>
          ...
          <Col md={6}>
            ...
            <Form.Select value={form.role ?? 'gallery'} onChange={(e) => setForm((s: any) => ({ ...s, role: e.target.value }))}>
              <option value="gallery">Gallery</option>
              <option value="hero">Hero</option>
              <option value="mobile_banner">Mobile Banner</option>
              <option value="thumbnail">Thumbnail</option>
              <option value="video_poster">Video</option>
            </Form.Select>
          </Col>
          <Col md={12}>
            <MediaPickerInput
              ...
              onChange={(fileId, file) =>
                setForm((s: any) => ({ ...s, mediaFileId: fileId, previewUrl: file?.url ?? null, previewMimeType: file?.mimeType ?? null }))
              }
              label="Media File"
              mimeTypePrefix={form.role === 'video_poster' ? 'video/' : 'image/'}
              accept={form.role === 'video_poster' ? 'video/*' : 'image/*'}
              helpText="Upload a valid image or video file. Use Replace if the selected file is invalid."
          ...
          <Col md={4}>
            ...
            <Form.Control value={form.altText ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, altText: e.target.value }))} />
          </Col>
          <Col md={6}>
      ...
      <Modal.Footer>
        ...
        <Button onClick={save} disabled={!form.mediaFileId}>
          Save
        </Button>
      </Modal.Footer>
    </Modal>
...
function DocumentEditor({
  ...
  show: boolean
  onHide: () => void
  item?: MembershipDocumentItem | null
  campaigns: MembershipCampaign[]
  onSaved: () => void
}) {
  const [form, setForm] = useState<any>({})
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>{item ? 'Edit Document' : 'Add Document'}</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Row className="g-3">
          <Col md={6}>
          ...
          <Col md={6}>
            ...
            <Form.Control value={form.documentType ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, documentType: e.target.value }))} />
          </Col>
          <Col md={12}>
            <MediaPickerInput
              value={form.mediaFileId}
          ...
          <Col md={6}>
            ...
            <Form.Control value={form.fileUrl ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, fileUrl: e.target.value }))} />
          </Col>
          <Col md={3}>
            <Form.Label>Sort Order</Form.Label>
            <Form.Control value={form.sortOrder ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, sortOrder: e.target.value }))} />
...
export default function MembershipMediaDocumentsContent() {
  ...
  return (
    <div className="container-fluid">
        ...
        <Col lg={6}>
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              {can('membership_media:create') && (
                <Button size="sm" onClick={() => setEditingMedia(null)}>
                  Add Media
                </Button>
              )}
            </Card.Header>
            <Card.Body>
              <LoadingOverlay loading={mediaLoading}>
                  ...
                  <tbody>
                    {mediaData?.data?.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>{item.role}</td>
                        <td>{item.mediaFile?.originalName ?? item.mediaFileId}</td>
                        <td>
                        ...
                        <td className="text-end d-flex gap-1 justify-content-end">
                          {can('membership_media:update') && (
                            <Button size="sm" variant="soft-primary" onClick={() => setEditingMedia(item)}>
                              Edit
                            </Button>
                          )}
                          {can('membership_media:delete') && (
                            <Button
                              size="sm"
                              variant="soft-danger"
                              onClick={async () => {
                                if (!(await confirmDelete('this media item'))) return
                                await membershipCampaignApi.deleteMedia(item.id)
                                refetchMedia()
                              }}>
                              Delete
                            </Button>
                ...
                {mediaData?.meta && mediaData.meta.totalPages > 1 && (
                  <Pagination
                    page={mediaData.meta.page}
                    limit={mediaData.meta.limit}
                    total={mediaData.meta.total}
                    totalPages={mediaData.meta.totalPages}
                    hasPrev={mediaData.meta.hasPrev}
                    hasNext={mediaData.meta.hasNext}
        ...
        <Col lg={6}>
          <Card>
            <Card.Header className="d-flex justify-content-between align-items-center">
              ...
              {can('membership_documents:create') && (
                <Button size="sm" onClick={() => setEditingDocument(null)}>
                  Add Document
                </Button>
              )}
            </Card.Header>
            <Card.Body>
              <LoadingOverlay loading={documentsLoading}>
                <Table hover className="align-middle">
                  <thead>
                    <tr>
                      <th>Campaign</th>
                  ...
                  <tbody>
                    {documentsData?.data?.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td className="text-end d-flex gap-1 justify-content-end">
                          {can('membership_documents:update') && (
                            <Button size="sm" variant="soft-primary" onClick={() => setEditingDocument(item)}>
                              Edit
                            </Button>
                          )}
                          {can('membership_documents:delete') && (
                            <Button
                              size="sm"
                              variant="soft-danger"
                              onClick={async () => {
                                if (!(await confirmDelete('this document'))) return
                                await membershipCampaignApi.deleteDocument(item.id)
                                refetchDocuments()
                              }}>
                              Delete
                            </Button>
      ...
      <MediaEditor
        show={editingMedia !== undefined}
        onHide={() => setEditingMedia(undefined)}
        item={editingMedia ?? null}
        campaigns={campaigns}
        onSaved={() => {
      ...
      />
      <DocumentEditor
        show={editingDocument !== undefined}
        onHide={() => setEditingDocument(undefined)}
        item={editingDocument ?? null}
        campaigns={campaigns}
        onSaved={() => {
