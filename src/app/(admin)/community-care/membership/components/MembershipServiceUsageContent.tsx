      ...
      <Card>
        <Card.Body>
          <InputGroup className="mb-3">
            ...
            <Form.Control
              placeholder="Search service code, service name, membership, or pet..."
              value={search}
                ...
                {data?.data?.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>
                      ...
                      <div className="small text-muted">{item.serviceCode}</div>
                    </td>
                    <td>{item.clinic?.name ?? '-'}</td>
                    <td>
                      <div>Regular: {item.regularPrice}</div>
                      <div className="small text-muted">
                        Discount: {item.discountAmount} | Payable: {item.payableAmount}
                      </div>
                    </td>
                    <td>
