...
function BenefitEditor({ show, onHide, item, campaigns, plans, onSaved }: { show: boolean; onHide: () => void; item?: MembershipBenefit | null; campaigns: MembershipCampaign[]; plans: MembershipPlan[]; onSaved: () => void }) {
  ...
  return (
    <Modal show={show} onHide={onHide} size="lg">
      <Modal.Header closeButton><Modal.Title>{item ? 'Edit Benefit' : 'Add Benefit'}</Modal.Title></Modal.Header>
      <Modal.Body>
        <Row className="g-3">
          <Col md={6}><Form.Label>Campaign</Form.Label><Form.Select value={form.campaignId ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, campaignId: e.target.value, planIds: [] }))}>{campaigns.map((campaign) => <option key={campaign.id} value={campaign.id}>{campaign.titleEn}</option>)}</Form.Select></Col>
          ...
          <Col md={4}><Form.Label>Icon</Form.Label><Form.Control value={form.icon ?? ''} onChange={(e) => setForm((s: any) => ({ ...s, icon: e.target.value }))} /></Col>
          ...
          <Col md={12}><Form.Label>Plans</Form.Label><div className="d-flex flex-wrap gap-3">{filteredPlans.map((plan) => <Form.Check key={plan.id} type="checkbox" label={`${plan.code} · ${plan.nameEn}`} checked={(form.planIds ?? []).includes(plan.id)} onChange={(e) => setForm((s: any) => ({ ...s, planIds: e.target.checked ? [...(s.planIds ?? []), plan.id] : (s.planIds ?? []).filter((id: string) => id !== plan.id) }))} />)}</div></Col>
        </Row>
      </Modal.Body>
...
export default function MembershipBenefitsContent() {
  ...
  const plansFn = useCallback(() => fetchAllPages<any>((p, l) => membershipCampaignApi.listPlans({ page: p, limit: l })).then(d => ({ data: d })), [page, limit])
  ...
  return (
    <div className="container-fluid">
      <PageHeader title="Benefits" breadcrumbs={[{ label: 'Membership Management' }, { label: 'Benefits' }]} action={can('membership_benefits:create') ? <Button onClick={() => setEditing(null)}>Add Benefit</Button> : undefined} />
      <ApiErrorAlert error={error as ApiError | null} />
      <Card><Card.Body><LoadingOverlay loading={loading}><Table hover className="align-middle"><thead><tr><th>Campaign</th><th>Title</th><th>Code</th><th>Plans</th><th>Status</th><th className="text-end">Actions</th></tr></thead><tbody>{data?.data?.map((item) => <tr key={item.id}><td>{item.campaign?.titleEn ?? item.campaignId}</td><td>{item.titleEn}</td><td>{item.code ?? '-'}</td><td>{item.plans?.length ?? 0}</td><td><StatusBadge status={item.isActive ? 'active' : 'inactive'} label={item.isActive ? 'Active' : 'Inactive'} /></td><td className="text-end d-flex gap-1 justify-content-end">{can('membership_benefits:update') && <Button size="sm" variant="soft-primary" onClick={() => setEditing(item)}>Edit</Button>}{can('membership_benefits:delete') && <Button size="sm" variant="soft-danger" onClick={async () => { if (!(await confirmDelete('this benefit'))) return; await membershipCampaignApi.deleteBenefit(item.id); refetch() }}>Delete</Button>}</td></tr>)}</tbody></Table>
{data?.meta && data.meta.totalPages > 1 && <Pagination page={data.meta.page} limit={data.meta.limit} total={data.meta.total} totalPages={data.meta.totalPages} hasPrev={data.meta.hasPrev} hasNext={data.meta.hasNext} onPageChange={setPage} onLimitChange={setLimit} />}
      ...
      <BenefitEditor show={editing !== undefined} onHide={() => setEditing(undefined)} item={editing ?? null} campaigns={campaignsData?.data ?? []} plans={plansData?.data ?? []} onSaved={() => { setEditing(undefined); refetch() }} />
    </div>
  )
}
