...
export default function TransparencyListContent() {
  ...
  const fetchFn = useCallback(
    () => transparencyReportsApi.list({ page, limit: 20, status: status || undefined }),
    [page, status],
  )
  ...
  async function handlePublish(id: string, isPublished: boolean) {
    await mutate(
      () => isPublished ? transparencyReportsApi.unpublish(id) : transparencyReportsApi.publish(id),
      undefined,
    )
    refetch()
  }
        ...
        <Card.Body>
          ...
          <LoadingOverlay loading={loading}>
            <Table hover className="table-centered align-middle mb-0">
              ...
              <tbody>
                {reports.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="text-center py-4 text-muted">No reports found</td>
                  </tr>
                ) : reports.map((report: TransparencyReport) => (
                  <tr
                    key={report.id}
                    style={{ cursor: 'pointer' }}
                    onClick={() => router.push(`/community-care/transparency/${report.id}`)}
                  >
                    <td>
          ...
          {meta && meta.totalPages > 1 && (
            <div className="d-flex justify-content-between align-items-center mt-3">
              <small className="text-muted">{meta.total} reports - Page {meta.page} of {meta.totalPages}</small>
