  ...
  return (
    <Dropdown className="topbar-item" show={open} onToggle={handleToggle}>
      <DropdownToggle
        ...
        {unread > 0 && (
          <Badge bg="danger" pill className="position-absolute top-0 start-100 translate-middle" style={{ fontSize: '0.65rem', minWidth: '1.2rem' }}>
            {unread > 99 ? '99+' : unread}
          </Badge>
        )}
      </DropdownToggle>
      ...
      <DropdownMenu className="py-0 dropdown-lg dropdown-menu-end" aria-labelledby="page-header-notifications-dropdown">
        ...
        <div style={{ maxHeight: '360px', overflowY: 'auto' }}>
          {loading && <div className="text-center py-3 text-muted small">Loading…</div>}
          ...
          {!loading &&
            items.map((n) => (
              <div key={n.id} className={`d-flex align-items-start p-3 border-bottom border-dashed gap-2 ${n.status === 'unread' ? 'bg-light' : ''}`}>
                <div
