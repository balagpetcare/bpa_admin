...
export default function ApiErrorAlert({ error, onDismiss }: ApiErrorAlertProps) {
  ...
  const mediaInUse = error instanceof ApiError && error.code === 'MEDIA_FILE_IN_USE'
    ? (error.details?.[0] as MediaFileInUseDetail | undefined)
    : null
      ...
      {details && (
        <ul className="mb-0 mt-1 ps-3">
          {(details as unknown[]).flatMap((d, i) => {
            ...
            const messageText = typeof (d as { message?: unknown }).message === 'string'
