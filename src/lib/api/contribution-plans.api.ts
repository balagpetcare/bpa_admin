...
export const contributionPlansApi = {
  list: (params?: PaginationQuery & { isActive?: boolean }) =>
    api.getPaginated<ContributionPlan>('/admin/contribution-plans', params),
  getById: (id: string) => api.get<ContributionPlan>(`/admin/contribution-plans/${id}`),
  create: (data: PlanCreatePayload) =>
    api.post<ContributionPlan>('/admin/contribution-plans', data),
  update: (id: string, data: PlanUpdatePayload) =>
    api.patch<ContributionPlan>(`/admin/contribution-plans/${id}`, data),
