...
import type {
  ...
  FieldOpsStats,
  QRScanLogEntry,
  VaccinationCompleteResult,
  CertificateIssueResult,
  CampaignFaq,
  CampaignContentMetadata,
  Participant,
  BulkSmsPreviewResult,
  BulkSmsBatch,
  BulkSmsFilters,
...
export interface CampaignCoverage {
  ...
  createdAt: string
  location: { id: string; type: LocationNodeType; nameEn: string; nameBn: string | null; parentId: string | null } | null
}

export const campaignsApi = {
  listVideos: (campaignId: string) => api.get<any[]>(`/admin/campaigns/${campaignId}/videos`),
  addVideo: (campaignId: string, data: any) => api.post<any>(`/admin/campaigns/${campaignId}/videos`, data),
  updateVideo: (campaignId: string, videoId: string, data: any) => api.patch<any>(`/admin/campaigns/${campaignId}/videos/${videoId}`, data),
  deleteVideo: (campaignId: string, videoId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/videos/${videoId}`),
  reorderVideos: (campaignId: string, videoIds: string[]) => api.post<void>(`/admin/campaigns/${campaignId}/videos/reorder`, { videoIds }),
  // Campaigns
  list: (params?: CampaignListParams) =>
    api.getPaginated<CampaignListItem>('/admin/campaigns', params as Record<string, string | number | boolean | undefined>),
  ...
  createSession: (campaignId: string, dto: CreateSessionDto) => api.post<CampaignSession>(`/admin/campaigns/${campaignId}/sessions`, dto),
  ...
  deleteSession: (campaignId: string, sessionId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/sessions/${sessionId}`),

  // Services
  listServices: (campaignId: string) => api.get<CampaignService[]>(`/admin/campaigns/${campaignId}/services`),
  createService: (campaignId: string, dto: CreateServiceDto) => api.post<CampaignService>(`/admin/campaigns/${campaignId}/services`, dto),
  ...
  deleteService: (campaignId: string, serviceId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/services/${serviceId}`),
  ...
  removeDoctorAssignment: (campaignId: string, assignmentId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/doctors/${assignmentId}`),
  bulkAssignDoctors: (
    campaignId: string,
    dto: {
      assignments: Array<{
  ...
  getAvailableDoctors: (campaignId: string, params?: { page?: number; limit?: number; search?: string; includeAssigned?: boolean }) =>
    api.getPaginated<Doctor>(`/admin/campaigns/${campaignId}/available-doctors`, params as Record<string, string | number | boolean | undefined>),
  removeDoctor: (campaignId: string, assignmentId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/doctors/${assignmentId}`),

  // Coverage Areas (location-first discovery)
  listCoverages: (campaignId: string) => api.get<CampaignCoverage[]>(`/admin/campaigns/${campaignId}/coverages`),
  addCoverage: (campaignId: string, dto: { locationId?: string; isNationwide?: boolean }) =>
    api.post<CampaignCoverage>(`/admin/campaigns/${campaignId}/coverages`, dto),
  removeCoverage: (campaignId: string, coverageId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/coverages/${coverageId}`),
  ...
  updateStaffAssignment: (
    campaignId: string,
    assignmentId: string,
    dto: { sessionId?: string | null; dutyRole?: StaffDutyRole; isActive?: boolean; notes?: string | null },
  ...
  bulkAssignStaff: (
    campaignId: string,
    dto: { assignments: Array<{ userId: string; sessionId?: string; dutyRole: StaffDutyRole; notes?: string | null }> },
  ...
  removeVolunteer: (campaignId: string, userId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/volunteers/${userId}`),
  ...
  deleteMedia: (campaignId: string, mediaId: string) => api.delete<void>(`/admin/campaigns/${campaignId}/media/${mediaId}`),
  reorderMedia: (campaignId: string, ids: string[]) => api.patch<CampaignMedia[]>(`/admin/campaigns/${campaignId}/media/reorder`, { ids }),
  ...
  checkIn: (
    campaignId: string,
    dto: { registrationId?: string; petBookingId?: string; token?: string; sessionId?: string; adminOverride?: boolean },
  ...
  getScanLogs: (
    campaignId: string,
    params?: { page?: number; limit?: number; sessionId?: string; scanResult?: string; dateFrom?: string; dateTo?: string },
  ) =>
    api.get<{ data: QRScanLogEntry[]; meta: { total: number; page: number; limit: number; totalPages: number } }>(
      `/admin/campaigns/${campaignId}/scan-logs`,
  ...
  getMyAssignedCampaigns: () =>
    api.get<{
      data: (CampaignListItem & {
        myAssignments: Array<{ id: string; dutyRole: StaffDutyRole; sessionId: string | null; session: CampaignSession | null; isActive: boolean }>
      })[]
    }>('/admin/my-assigned-campaigns'),
  ...
  getPaymentSummary: (campaignId: string) =>
    api.get<Record<string, { count: number; total: number }>>(`/admin/campaigns/${campaignId}/participants/payment-summary`),

  bulkSmsPreview: (campaignId: string, dto: { template: string; filters: BulkSmsFilters }) =>
    api.post<BulkSmsPreviewResult>(`/admin/campaigns/${campaignId}/participants/bulk-sms/preview`, dto),

  bulkSmsSend: (campaignId: string, dto: { template: string; filters: BulkSmsFilters; previewCount: number; confirmation: true }) =>
    api.post<{ batchId: string; recipientCount: number; queued: number }>(`/admin/campaigns/${campaignId}/participants/bulk-sms/send`, dto),

  bulkSmsHistory: (campaignId: string) => api.get<BulkSmsBatch[]>(`/admin/campaigns/${campaignId}/participants/bulk-sms/history`),
