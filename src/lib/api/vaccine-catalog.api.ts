...
export const vaccineCatalogApi = {
  ...
  list: (params?: VaccineCatalogListParams) =>
    api.get<PaginatedResult<VaccineCatalog>>('/admin/vaccine-catalog', params as Record<string, string | number | boolean | undefined>),
  getById: (id: string) => api.get<VaccineCatalog>(`/admin/vaccine-catalog/${id}`),
  create: (dto: { name: string; species?: string; standardIntervalDays?: number; manufacturer?: string; description?: string }) =>
    api.post<VaccineCatalog>('/admin/vaccine-catalog', dto),
  update: (
    id: string,
    dto: Partial<{ name: string; species: string; standardIntervalDays: number; manufacturer: string; description: string; isActive: boolean }>,
  ) => api.patch<VaccineCatalog>(`/admin/vaccine-catalog/${id}`, dto),
  remove: (id: string) => api.delete<void>(`/admin/vaccine-catalog/${id}`),

  // Certificate Templates
  listTemplates: () => api.get<CertificateTemplate[]>('/admin/vaccine-catalog/certificate-templates'),
  createTemplate: (dto: { name: string; htmlTemplate: string }) => api.post<CertificateTemplate>('/admin/vaccine-catalog/certificate-templates', dto),
